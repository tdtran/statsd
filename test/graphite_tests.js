  ...
  send_well_formed_posts: function (test) {
    ...
    this.acceptor.once('connection',function(c){
      ...
      c.on('end',function(){
        ...
        test.ok(_.include(_.map(entries,function(x) { return _.keys(x)[0] }),'stats_counts.numStats'),'graphite output includes numStats');
        test.equal(_.find(entries, function(x) { return _.keys(x)[0] == 'stats_counts.numStats' })['stats_counts.numStats'],0);
        test.done();
      });
    });
  },
      ...
      statsd_send('a_test_value:' + testvalue + '|ms',me.sock,'127.0.0.1',8125,function(){
          collect_for(me.acceptor,me.myflush*2,function(strings){
            ...
            var numstat_test = function(post){
              var mykey = 'stats_counts.numStats';
              return _.include(_.keys(post),mykey) && (post[mykey] == 1);
            };
            test.ok(_.any(hashes,numstat_test), 'stats_counts.numStats should be 1');
